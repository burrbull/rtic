...
#h1((en: [RTIC by example]
))

This part of the book introduces the RTIC framework to new users by
walking them through examples of increasing complexity.

All examples in this part of the book are part of the
#link("https://github.com/rtic-rs/rtic/tree/master/rtic/examples")[RTIC repository],
found in the `examples` directory. The examples are runnable on QEMU
(emulating a Cortex M3 target), thus no special hardware required to
follow along.
...
for instructions on how to set up an embedded development environment
that includes QEMU.
...
This runs all of the examples against the default `thumbv7m-none-eabi`
device `lm3s6965`.

To limit which examples are being run, use the flag
`--example <example name>`, the name being the filename of the example.
...
It is great that examples are passing and this is part of the RTIC CI
...
currently `lm3s6965` is the best supported, work is ongoing to increase
support for other devices, including both ARM and RISC-V
]
