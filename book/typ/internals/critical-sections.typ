...
#h1((en: [Critical sections]
), offset: whole)

When a resource (static variable) is shared between two, or more, tasks
that run at different priorities some form of mutual exclusion is
required to mutate the memory in a data race free manner. In RTIC we use
...
How high must the dynamic priority be to ensure mutual exclusion on a
particular resource? The #link("ceilings.html")[ceiling analysis] is in
charge of answering that question and will be discussed in the next
section. This section will focus on the implementation of the critical
section.
...
section when it needs to modify the shared memory. On the other hand,
the higher priority task can directly modify the shared memory because
it can't be preempted by the lower priority task. To enforce the use of
a critical section on the lower priority task we give it a
#emph[resource proxy], whereas we give a unique reference (`&mut-`) to
...
Where `static_priority` is the priority programmed in the NVIC for the
current interrupt, or a logical `0` when the current context is `idle`.

In this particular example we could implement the critical section as
follows:

#quote(block: true)[
#strong[NOTE:] this is a simplified implementation
]
...
Here it's important to use the `"memory"` clobber in the `asm!` block.
It prevents the compiler from reordering memory operations across it.
This is important because accessing the variable `x` outside the
critical section would result in a data race.

It's important to note that the signature of the `lock` method prevents
nesting calls to it. This is required for memory safety, as nested calls
would produce multiple unique references (`&mut-`) to `x` breaking Rust
...
that end we'll track the dynamic priority of the task using a stack
variable and use that to decide whether to write to `BASEPRI` or not. In
practice, the stack variable will be optimized away by the compiler but
it still provides extra information to the compiler.
...
At the end the compiler will optimize the function `foo` into something
like this:
...
This invariant needs to be preserved to avoid raising the dynamic
priority of a handler through preemption. This is best observed in the
following example:
...
The consequence is that `idle` will run at a dynamic priority of `2` and
in fact the system will never again run at a dynamic priority lower than
`2`. This doesn't compromise the memory safety of the program but
affects task scheduling: in this particular case tasks with a priority
of `1` will never get a chance to run.
