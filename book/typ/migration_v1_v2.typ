...
following steps:
+ `v2.1.0` works on Rust Stable from 1.75 (*recommended*), while
...
#link(<migration_v1_v2_example>)[the full example migration page].
...
+ Instead of `spawn_after` and `spawn_at`, you now use the `async`
  ...
  `rtic-monotonics`.
+ Software tasks _must_ be `async fn`s now. Not returning from a
