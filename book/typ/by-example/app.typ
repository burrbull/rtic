...
#h1((en: [The `#[app]` attribute and an RTIC application]
), offset: whole)
...
The `app` attribute will expand into a suitable entry point and thus
replaces the use of the
#link("https://docs.rs/cortex-m-rt-macros/latest/cortex_m_rt_macros/attr.entry.html")[`cortex_m_rt::entry`]
attribute.
...
- Tasks (both hardware and software) run preemptively based on their
  associated static priority.
- Hardware tasks are bound to underlying hardware interrupts.
- Software tasks are schedulied by an set of asynchronous executors, one
  for each software task priority.
- `idle` has the lowest priority, and can be used for background work,
  and/or to put the system to sleep until it is woken by some event.
...
Overall, the generated code infers no additional overhead in comparison
to a hand-written implementation, thus in Rust terms RTIC offers a
...
specified, it is set to the default value of 0.

Priorities in RTIC follow a higher value = more important scheme. For
